...
export class TINFMarch2023 extends VoFarmStrategy {

    protected historyLength = 5
    ...
    private execute(): void {
        ...
        for (const positionInsightsEntry of this.positionInsights) {
            ...
            console.log(`Hallo TINF ${position.data.size} (${positionValue} - ${percentageOfEquity}%) ${positionInsightsEntry.tradingPair} ${pnl} ${enhancePositionTrigger} ${reducePositionTrigger}`)
            
            if (positionInsightsEntry.tradingPair === "BTCUSDT") {
                this.addInvestmentAdvice(Action.BUY, positionInsightsEntry.tradingUnit, positionInsightsEntry.tradingPair, "buying for demo reasons")
            }
